...
<head>
    ...
    <link rel="stylesheet" href="${ctx}/css/admin-dashboard.css?v=5" />
    <title>Orders — Booktify Staff</title>
</head>
    ...
    <main class="admin-main">
        ...
        <section class="admin-content">
            ...
            <div class="admin-panel" style="padding:14px 22px;">
                <form method="get" action="${ctx}/staff/orders" class="admin-search-form">
                    ...
                    <a href="${ctx}/staff/orders" class="admin-button admin-button--ghost"><i class="fa-solid fa-rotate-right"></i> Reset</a>
                </form>
            </div>
                    ...
                    <c:if test="${totalPages > 1}">
                        <div class="admin-pagination__nav">
                            <a class="pag-btn ${currentPage == 0 ? 'pag-btn--disabled' : ''}"
                               href="${ctx}/staff/orders?page=${currentPage - 1}&keyword=<c:out value='${keyword}'/>&status=<c:out value='${status}'/>&sort=<c:out value='${sort}'/>">
                                <i class="fa-solid fa-chevron-left" style="font-size:.7rem;"></i>
                            </a>
                            <c:forEach begin="0" end="${totalPages - 1}" var="i">
                                <a class="pag-btn ${i == currentPage ? 'pag-btn--active' : ''}"
                                   href="${ctx}/staff/orders?page=${i}&keyword=<c:out value='${keyword}'/>&status=<c:out value='${status}'/>&sort=<c:out value='${sort}'/>">
                                    ${i + 1}
                                </a>
                            </c:forEach>
