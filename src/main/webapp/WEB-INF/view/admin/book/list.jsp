<%-- Book list with search, category/genre/status filters, pagination, and two modals (view details, confirm
    delete/restore). --%>
            ...
            <body class="admin-shell">
                ...
                <main class="admin-main">
                    ...
                    <section class="admin-content">
                        ...
                        <%-- Filter Bar: plain GET form, each field becomes a query param the controller re-reads so
                                ...
                                <form method="get" action="/admin/books" class="admin-search-form"
                                    ...
                                    <div style="position:relative;flex:1;max-width:380px;">
                                        ...
                                        <input type="text" name="q" value="<c:out value='${q}'/>"
                                            placeholder="Search by title, author, ISBN, category or genre..."
                                            class="admin-input" style="padding-left:38px;" />
                                    </div>
                                    <select name="status" class="admin-input" style="max-width:160px;">
                                        <option value="" <c:if test="${empty status}">selected</c:if>>All Status
                                        </option>
                                    ...
                                    <select name="categoryId" class="admin-input" style="max-width:180px;">
                                        ...
                                        <c:forEach items="${categories}" var="cat">
                                            <option value="${cat.id}" <c:if test="${selectedCategoryId == cat.id}">
                                                selected</c:if>>
                                    ...
                                    <div class="genre-filter-wrap" id="genreFilterWrap">
                                        <button type="button" class="genre-filter-control admin-input"
                                            id="genreFilterControl" title="Filter by Genre">
                                            <span id="genreFilterLabel">
                                                <c:choose>
                                                    <c:when test="${empty selectedGenreIds}">All Genres</c:when>
                                                    <c:otherwise>${selectedGenreIds.size()}
                                                        genre${selectedGenreIds.size() > 1 ? 's' : ''}</c:otherwise>
                                                </c:choose>
                                            </span>
                                                    ...
                                                    <c:forEach items="${genres}" var="g">
                                                        <label class="genre-filter-option">
                                                            <input type="checkbox" name="genreIds" value="${g.id}"
                                                                class="genre-filter-checkbox" <c:if
                                                                test="${selectedGenreIds.contains(g.id)}">checked</c:if>
                                                            />
                                                            <span>
                                                                <c:out value="${g.name}" />
                                                            </span>
                                    ...
                                    <button type="submit" class="admin-button"><i class="fa-solid fa-filter"></i>
                                        Filter</button>
                                    <a href="/admin/books" class="admin-button admin-button--ghost"><i
                                    ...
                                    <tbody>
                                        <c:forEach items="${books}" var="book" varStatus="vs">
                                            <tr>
                                                ...
                                                <td>
                                                    <c:choose>
                                                        <c:when test="${not empty book.category}">
                                                            <span class="cat-badge"><i class="fa-solid fa-tag"
                                                ...
                                                <td style="max-width:220px;">
                                                    <c:choose>
                                                        <c:when test="${not empty book.genreNames}">
                                                            <span style="color:#374151;font-size:.82rem;">
                                                                <c:out value="${book.genreNames}" />
                                                            </span>
                                                        </c:when>
                                                        <c:otherwise><span style="color:#9CA3AF;">&#8212;</span>
                                                        </c:otherwise>
                                                ...
                                                <td>
                                                    <span
                                                        class="status-pill ${book.active ? 'status-pill--on' : 'status-pill--off'}">
                                                ...
                                                <td class="admin-table__actions">
                                                    ...
                                                    <c:choose>
                                                        ...
                                                        <c:otherwise>
                                                            <button type="button" class="icon-link icon-link--restore"
                                                                title="Restore"
                                    ...
                                    <div class="admin-pagination">
                                        <div class="admin-pagination__info">
                                            <c:choose>
                                                ...
                                                <c:otherwise>
                                                    Showing <strong>${fromItem}</strong> to <strong>${toItem}</strong>
                                                    of <strong>${totalItems}</strong> entr${totalItems == 1 ? 'y' :
                                                    'ies'}<c:if
                                                        test="${not empty q or not empty status or not empty selectedCategoryId or not empty selectedGenreIds}">
                                                        (filtered)</c:if>
                                                </c:otherwise>
                                            </c:choose>
                                        </div>
                                        <c:if test="${totalPages > 1}">
                                            <%-- Rebuild the query string so page links keep the active filters. --%>
                                                <c:set var="genreQS">
                                                    <c:forEach items="${selectedGenreIds}" var="gid">
                                                        &amp;genreIds=${gid}</c:forEach>
                                                </c:set>
                                                <c:set var="pagBase"
                                                    value="/admin/books?q=${q}&status=${status}&categoryId=${selectedCategoryId}${genreQS}" />
                    ...
                    <div id="confirmModal" class="modal-overlay" style="display:none;" onclick="closeConfirmModal()">
                        <div class="modal-box" style="max-width:420px;" onclick="event.stopPropagation()">
                            ...
                            <div class="modal-body" style="display:block;">
                                <p id="confirmModalMsg"
                                    style="margin:0;font-size:.9rem;color:#374151;line-height:1.65;"></p>
                            </div>
                        ...
                        <div id="bookModal" class="modal-overlay" style="display:none;"
                            ...
                            <div class="modal-box" style="max-width:680px;" onclick="event.stopPropagation()">
                                <div class="modal-header">
                                    ...
                                    <button class="modal-close" onclick="closeModal('bookModal')"><i
                                ...
                                <div class="modal-body">
                                    <div class="book-modal-body">
                                        ...
                                        <div class="book-modal-info">
                                            <div class="modal-row"><span class="modal-label">Title</span><span
                                                    id="mBTitle" class="modal-value"></span></div>
                                            <div class="modal-row"><span class="modal-label">Author</span><span
                                                    id="mBAuthor" class="modal-value"></span></div>
                                            <div class="modal-row"><span class="modal-label">ISBN</span><span
                                            ...
                                            <div class="modal-row"><span class="modal-label">Price</span><span
                                                    id="mBPrice" class="modal-value"></span></div>
                                            <div class="modal-row"><span class="modal-label">Stock</span><span
                                                    id="mBStock" class="modal-value"></span></div>
                                            <div class="modal-row"><span class="modal-label">Status</span><span
                                                    id="mBStatus" class="modal-value"></span></div>
                                            <div class="modal-row"><span class="modal-label">Description</span><span
                                                    id="mBDesc" class="modal-value"
                                                    style="white-space:pre-line;"></span></div>
                                            <div class="modal-row"><span class="modal-label">Created</span><span
                                                    id="mBCreated" class="modal-value"></span></div>
                                            <div class="modal-row"><span class="modal-label">Updated</span><span
                                                    id="mBUpdated" class="modal-value"></span></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
