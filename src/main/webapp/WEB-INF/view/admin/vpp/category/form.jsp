<%-- Form tạo/sửa Stationery (VPP) Category — layout đồng nhất với Book Category
     (admin-form-card + admin-field + admin-form__actions). Dùng chung cho cả 2
     mode (formMode="create"|"edit"), action đổi theo mode: POST /admin/vpp/categories
     (create) hoặc POST /admin/vpp/categories/{id} (edit). --%>
<%@ page contentType="text/html" pageEncoding="UTF-8" %>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ taglib prefix="form" uri="http://www.springframework.org/tags/form" %>
<c:choose>
    <c:when test="${formMode == 'edit'}">
        <c:url var="formAction" value="/admin/vpp/categories/${categoryId}" />
    </c:when>
    <c:otherwise>
        <c:url var="formAction" value="/admin/vpp/categories" />
    </c:otherwise>
</c:choose>
<!DOCTYPE html>
<html lang="en">
...
<head>
    ...
    <title>
        <c:choose>
            <c:when test="${formMode=='edit'}">Edit Stationery Category</c:when>
            <c:otherwise>Create Stationery Category</c:otherwise>
    ...
    <main class="admin-main">
        ...
        <section class="admin-content">
            ...
            <div class="admin-toolbar">
                ...
                <a href="/admin/categories?tab=stationery" class="admin-button admin-button--ghost">
            ...
            <div class="admin-form-card">
                ...
                <div class="admin-form-card__body">
                    <form:form modelAttribute="categoryDTO" action="${formAction}" method="post"
                        class="admin-form" id="categoryForm">
                                ...
                                <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;padding:14px 16px;background:#F9FAFB;border:1px solid #E5E7EB;border-radius:8px;">
                                    <div>
                                        ...
                                        <div style="font-size:.875rem;color:#374151;font-weight:500;">
                                            <c:out value="${categoryDTO.createdAtFormatted}" default="—" />
                                        </div>
                                    </div>
                                    <div>
                                        ...
                                        <div style="font-size:.875rem;color:#374151;font-weight:500;">
                                            <c:out value="${categoryDTO.updatedAtFormatted}" default="—" />
                                        </div>
                                    </div>
                                </div>
                            </div>
