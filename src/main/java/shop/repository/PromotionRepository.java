...
public interface PromotionRepository extends JpaRepository<Promotion, Long> {
    ...
    @Query("""
            ...
            """)
    Optional<Promotion> findByIdWithCategories(@Param("promotionId") Long promotionId);
    ...
    @Query("""
            ...
            WHERE promotion.active = true
              AND category.id = :categoryId
              AND promotion.isPercentage = :percentage
              AND promotion.startDate <= CURRENT_TIMESTAMP
              AND promotion.endDate >= CURRENT_TIMESTAMP
            ORDER BY promotion.endDate ASC, promotion.id ASC
            """)
    List<Promotion> findActivePromotionsByCategoryIdAndType(
            @Param("categoryId") Long categoryId,
            @Param("percentage") boolean percentage);
    ...
    @Query("""
            ...
            WHERE promotion.active = true
              ...
              AND (:excludePromotionId IS NULL OR promotion.id <> :excludePromotionId)
            """)
