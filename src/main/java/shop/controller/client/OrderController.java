...
public class OrderController {
    ...
    public String placeOrder(
        ...
        User user =
                getCurrentUser(authentication); // Email → User entity
        ...
        PaymentMethod paymentMethod =
                checkoutForm.resolvePaymentMethod();
        ...
        try {
            ...
            if (paymentMethod == PaymentMethod.VNPAY) {
                shop.domain.Order paymentOrder =
                        orderService.getOrderEntityForPayment(
                                order.getId()
                        );
    ...
    public String orderHistory(
        ...
        User user =
                getCurrentUser(authentication); // Lấy User từ email trong Authentication
    ...
    public String orderDetail(
        ...
        User user =
                getCurrentUser(authentication); // Lấy User entity từ session
        ...
        return orderService
                ...
                .orElseGet(() -> { // Optional rỗng — không tìm thấy hoặc không thuộc user
                    redirectAttributes
                            .addFlashAttribute(
                                    "errorMessage",
                                    "Order not found." // Hiện trên trang /orders sau redirect
                            );
    ...
    public String cancelOrder(
        ...
        User user =
                getCurrentUser(authentication); // User đang đăng nhập
    ...
    private void initializePromotionSelections(
        ...
        rawCart.getItems().forEach(
                cartItem -> { // Lambda duyệt từng dòng giỏ
                    Book book =
                            cartItem.getBook(); // Chỉ sách mới có KM per-item
    ...
    // Đọc map promotion selections từ session (key CartController.PROMOTION_SESSION_KEY)
    ...
    private void applyCartPromotionSelections(
        ...
        // Session lưu Map<Long, String> → copy vào checkoutForm (tránh sửa session trực tiếp)
